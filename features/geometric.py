...
def calculate_spatial_features(layout: Layout) -> Dict[str, float]:
    ...
    if total_area > 0:
        ...
        com_y = sum(y * area for y, area in zip(centroids_y, areas)) / total_area
    else:
        com_x = np.mean(centroids_x)
        com_y = np.mean(centroids_y)
    ...
    spread_x = np.std(centroids_x) / layout.width if len(centroids_x) > 1 and layout.width > 0 else 0.0
    spread_y = np.std(centroids_y) / layout.height if len(centroids_y) > 1 and layout.height > 0 else 0.0
...
def calculate_balance_features(layout: Layout) -> Dict[str, float]:
    ...
    weight_distribution_x = np.std(weighted_positions_x) if len(weighted_positions_x) > 1 else 0.0
    weight_distribution_y = np.std(weighted_positions_y) if len(weighted_positions_y) > 1 else 0.0
...
def calculate_hierarchy_features(layout: Layout) -> Dict[str, float]:
    ...
    size_variance = np.std(areas) / (area_mean + 1e-6)
...
def calculate_flow_features(layout: Layout) -> Dict[str, float]:
    ...
    if len(elements) < 2:
        ...
        element_positions = [
            (elem.x + elem.width / 2, elem.y + elem.height / 2, elem.area) 
            for elem in elements
        ]
    ...
    if len(zone_areas) > 0 and sum(zone_areas) > 0:
        ...
        scanning_pattern = 1.0 - (np.std(zone_areas) / (zone_mean + 1e-6))
    else:
        scanning_pattern = 0.0
    ...
    return {
        ...
        'scanning_pattern': max(0.0, scanning_pattern),
    }


def extract_all_features(layout: Layout) -> Dict[str, float]:
    """
    ...
    try:
        ...
        features['elements_per_area'] = features['element_count'] / (layout.width * layout.height / 10000)
        features['visual_complexity'] = features['element_count'] * features['size_variance']
